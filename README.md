...
│       │       │   └── VisitorServlet.java      # 访客计数业务逻辑
│       │       ├── filter/
│       │       │   └── EncodingFilter.java      # 字符编码过滤器
│       │       ├── dao/
│       │       │   ├── MessageDAO.java          # 消息数据访问层
│       │       │   └── VisitorDAO.java          # 访客数据访问层
│       │       ├── entity/
│       │       │   └── Message.java             # 消息实体类
│       │       └── util/
│       │           └── DBUtil.java              # 数据库工具类
...
6. ✅ **MVC 架构**：分层设计、职责分离（Controller-Service-DAO）
7. ✅ **安全防护**：XSS 防护、输入验证
8. ✅ **线程安全**：并发控制、同步机制
9. ✅ **数据库操作**：JDBC 使用、DAO 模式、事务管理
10. ✅ **连接池技术**：HikariCP 配置和使用
11. ✅ **Maven 构建**：依赖管理、项目构建
12. ✅ **Web 配置**：web.xml 配置、欢迎页、错误页
13. ✅ **CSS 模块化**：样式文件分离和管理
